...
function getAccessToken(): string {
  ...
  if (!token) {
    throw new Error("STRATEEGIA_ACCESS_TOKEN environment variable is not set");
  }
  return token;
}
...
Status: ${project.active ? "Active" : "Inactive"}${project.finished ? " (Finished)" : ""}
...
server.tool(
  ...
  async () => {
    try {
      ...
      const projects: StrateegiaProject[] = await fetchStrateegiaAPI("/projects/v1/project");
